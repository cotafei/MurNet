...
from core.crypto import Identity, E2EEncryption, ValidationError
from core.transport import Transport, PacketType, RateLimiter
from core.routing import RoutingTable, LSA, LinkStateDatabase
from core.dht_rpc import DHTAuthManager, DHTRequest
...
class TestCryptoSecurity:
    ...
    def test_invalid_base64_handling(self, identity):
        """Обработка невалидного base64"""
        data = {'test': 'data'}
        
        with pytest.raises((ValidationError, Exception)):
            identity.verify(data, "not-valid-base64!!!", "also-invalid")
...
class TestTransportSecurity:
    ...
    def test_replay_protection_sequence_numbers(self, fresh_data_dir):
        ...
        from core.transport import PeerConnection
        ...
        # Старый sequence отклонен
        assert peer.is_sequence_valid(0) is False
    ...
    def test_timestamp_validation(self, fresh_data_dir):
        ...
        from core.transport import PacketHeader, PacketType
...
class TestRoutingSecurity:
    ...
    def test_trust_score_degradation(self, identity):
        ...
        # Много невалидных LSA
        for i in range(10):
            lsa = LSA(
                origin=other.address,
                sequence=-i,  # Невалидный sequence
                links={},
                timestamp=time.time()
            )
...
class TestNodeSecurity:
    ...
    def test_circuit_breaker_blocks_under_load(self, fresh_data_dir):
        ...
        from core.node import CircuitBreaker
    ...
    def test_backpressure_throttling(self, fresh_data_dir):
        ...
        from core.node import BackpressureController
...
class TestDoSProtection:
    ...
    def test_memory_exhaustion_protection(self, fresh_data_dir):
        ...
        from core.murnaked import BloomFilter
...
class TestSideChannelProtection:
    ...
    def test_no_timing_leak_in_verification(self, identity):
        ...
        # Время должно быть примерно одинаковым (в пределах 50%)
        ratio = max(time_correct, time_wrong) / min(time_correct, time_wrong)
        assert ratio < 1.5  # Не более 50% разницы
