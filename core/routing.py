...
from core.crypto import blake2b_hash, canonical_json
    ...
    def compute(self, source: str, min_trust: float = 0.3) -> Dict[str, Path]:
        ...
        while pq:
            ...
            for neighbor, link in graph.get(current, {}).items():
                ...
                trust = self.lsdb.get_trust_score(neighbor)
                adjusted_cost = cost / trust  # Higher cost for untrusted nodes
                
                new_dist = dist + adjusted_cost
