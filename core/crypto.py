...
def base58_decode(data: str) -> bytes:
    ...
    if not data or len(data) > 100:  # Max reasonable address length
    ...
    leading_ones = sum(1 for ch in data if ch == '1')
    num = 0
    for ch in data:
...
class Identity:
    ...
    def _init_from_seed(self, seed: bytes):
        """Initialize from seed using Argon2id"""
        if len(seed) < 16:
            raise ValidationError("Seed must be at least 16 bytes")
        
        # Use Argon2id instead of simple SHA256
        key, _ = derive_key_argon2(seed)
    ...
    def verify(self, data: dict, signature_b64: str, public_key_b64: str) -> bool:
        """Verify signature with constant-time comparison"""
        try:
            public_bytes = safe_base64_decode(public_key_b64)
...
class E2EEncryption:
    ...
    def encrypt_message(self, plaintext: str, recipient_public_key: bytes) -> dict:
        ...
        return {
            ...
            'sender_pubkey': base64.b64encode(self.identity.get_public_bytes()).decode(),
            'version': 'v5'
        }
