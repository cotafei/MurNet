...
from core.crypto import blake2b_hash, secure_random_bytes, constant_time_compare
...
class PacketHeader:
    ...
    SIZE = 22  # Without auth_tag
    FULL_SIZE = 38  # With auth_tag
...
class PeerConnection:
    ...
    # Rate limiting
    packet_times: deque = field(default_factory=lambda: deque(maxlen=100))
...
class Transport:
    ...
    def _send_hello(self, addr: Tuple[str, int]):
        ...
        hello_data = {
            ...
            'public_key': self._public_key.hex() if self._public_key else '',
            'timestamp': int(time.time()),
