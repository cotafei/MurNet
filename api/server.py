...
from core.crypto import blake2b_hash, secure_random_bytes
...
class MurnetAPIServer:
    ...
    def _setup_routes(self):
        ...
        async def upload_file(
            ...
            if to_address:
                self.node.send_message(to_address, f"[FILE:{safe_name}:{result['file_id']}]")
        ...
        async def connect_peer(
            ...
            success = self.node.transport.connect_to(
                request.ip, request.port, request.address
            )
    ...
    def _build_full_status(self) -> Dict:
        ...
        return {
            ...
            "node": {
                ...
                "version": "5.0-secure",
                "uptime_seconds": int(time.time() - node_status['stats']['start_time']),
                "peers_count": node_status.get('neighbors', 0),
                "connections_active": len(self.node.transport.peers),
